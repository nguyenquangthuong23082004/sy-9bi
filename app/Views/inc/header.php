	...
	<div class="header-bottom">
		...
		<<?= $syLogoTag ?> class="logo">
			<a href="<?= $syLogoHref ?>" class="logo-link">
				<img src="<?= $syHeaderLogoSrc ?>" alt="<?= esc($sySiteName) ?><?= $isHome ? '' : ' 홈으로 이동' ?>"
					class="logo-img">
			</a>
		</<?= $syLogoTag ?>>
												...
												<ul class="gnb-depth-list">
													<?php foreach ($group as $item): ?>
														<li class="gnb-depth-item">
															...
															<?php else: ?>
																<a href="<?= sy_nav_url($item['url'], $isHome) ?>" class="gnb-depth-link"
																	<?= ($isCurrent && !empty($item['key']) && $item['key'] === $depthCurrent) ? ' aria-current="page"' : '' ?>><?= esc($item['label']) ?></a>
															<?php endif; ?>

															<?php if (!empty($item['sub'])): ?>
																<ul
																	class="<?= !empty($item['subCol']) ? 'gnb-depth2-col-list' : 'gnb-depth2-list' ?>">
																	<?php foreach ($item['sub'] as $sub): ?>
																		<li
																			class="<?= !empty($item['subCol']) ? 'gnb-depth2-col-item' : 'gnb-depth2-item' ?>">
																			<?= esc($sub) ?>
																		</li>
																	<?php endforeach; ?>
																</ul>
		...
		<div class="header-bottom-right">
			<!-- <a href="<?= base_url() ?>" class="btn btn-language<?= $isHome ? '' : ' only-pc' ?>"
				aria-label="Change language to English">EN</a> -->
			<button type="button" class="btn btn-gnb js-btn-gnb" aria-label="메뉴 열기/닫기" aria-expanded="false"
