...
<div class="card shadow-sm mb-4">
    <div class="card-header bg-white py-3">
        <h5 class="card-title mb-0 ">
            <i class="bi bi-pencil-square me-1"></i> <?= $config['board_name'] ?>
            <?= !empty($item['bbs_idx']) ? '수정' : '등록' ?>
        </h5>
    </div>
    <div class="card-body">
        <form name="frm" id="frm" action="<?= base_url('AdmMaster/bbs/' . $code . '/save') ?>" method="post"
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6" <?= $hide_fields ? 'style="display:none"' : '' ?>>
                    ...
                    <input type="text" name="writer" value="<?= esc($item['writer'] ?? '관리자') ?>"
                        class="form-control" />
                </div>
                ...
                <?php if ($skin == 'event'): ?>
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <input type="text" name="s_date" value="<?= esc($item['s_date'] ?? '') ?>"
                                class="datepicker form-control" placeholder="시작일" />
                            <span class="input-group-text">~</span>
                            <input type="text" name="e_date" value="<?= esc($item['e_date'] ?? '') ?>"
                                class="datepicker form-control" placeholder="종료일" />
                        </div>
                    </div>
                <?php endif; ?>
                        ...
                        <div class="p-2 border rounded bg-light w-100">
                            <?php if ($isNotice == 'Y'): ?>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="notice_yn" id="notice_yn" value="Y"
                                        <?= ($item['notice_yn'] ?? '') == 'Y' ? 'checked' : '' ?>>
                                    <label class="form-check-label" for="notice_yn">공지글</label>
                                </div>
                            <?php endif; ?>
                            <?php if ($isSecure == 'Y'): ?>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="secure_yn" id="secure_yn" value="Y"
                                        <?= ($item['secure_yn'] ?? '') == 'Y' ? 'checked' : '' ?>>
                ...
                <div class="col-md-3" <?= (in_array($skin, ['faq', 'media', 'event']) || $code == 'banner') ? 'style="display:none"' : '' ?>>
                    ...
                    <input type="text" name="r_date" value="<?= esc($item['r_date'] ?? date('Y-m-d H:i:s')) ?>"
                ...
                <?php if ($code == 'banner'): ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="subject" value="<?= esc($item['subject'] ?? '') ?>" class="form-control"
                            style="height: 38px !important;" placeholder="배너 제목 (빈칸 시 미노출)" />
                    </div>
                <?php else: ?>
                    <!-- 제목 -->
                ...
                <?php if ($code == 'app' || $code == 'faq'): ?>
                    <div class="col-md-2">
                        ...
                        <input type="number" name="onum" value="<?= esc($item['onum'] ?? '0') ?>"
                            class="form-control text-center" style="height: 38px !important;" />
                    </div>
                <?php endif; ?>
                ...
                <?php if ($code == 'banner'): ?>
                    <div class="col-12">
                        ...
                        <input type="text" name="sub_title" value="<?= esc($item['sub_title'] ?? '') ?>"
                            class="form-control" placeholder="상단 소제목을 입력하세요 (빈칸 시 미노출)" />
                    </div>

                    <div class="col-12">
                        <label class="form-label fw-bold">하단 설명문</label>
                        <textarea name="contents" class="form-control" rows="3"
                            placeholder="하단 설명문을 입력하세요 (빈칸 시 미노출)"><?= esc($item['contents'] ?? '') ?></textarea>
                    </div>

                    <div class="col-12">
                        <label class="form-label fw-bold">링크 URL</label>
                        <input type="text" name="url" value="<?= esc($item['url'] ?? '') ?>" class="form-control"
                            placeholder="http://" />
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-bold">버튼 문구</label>
                        <input type="text" name="btn_text" value="<?= esc($item['btn_text'] ?? '') ?>" class="form-control"
                            placeholder="예: 자세히 보기 (빈칸 시 버튼 미노출)" />
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-bold d-block">의료진 자료 요청 버튼</label>
                        <div class="p-2 border rounded bg-light" style="height: 38px; display: flex; align-items: center;">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="btn_medical_yn" id="btnMedicalYn"
                                    value="Y" <?= ($item['btn_medical_yn'] ?? 'N') == 'Y' ? 'checked' : '' ?>>
                                <label class="form-check-label fw-bold" for="btnMedicalYn">
                                    '의료진 자료 요청' 버튼 함께 노출
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="p-2 border rounded bg-light">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="notice_yn" id="noticeY" value="Y"
                                    <?= ($item['notice_yn'] ?? 'Y') == 'Y' ? 'checked' : '' ?>>
                                <label class="form-check-label" for="noticeY">노출함</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="notice_yn" id="noticeN" value="N"
                                    <?= ($item['notice_yn'] ?? '') == 'N' ? 'checked' : '' ?>>
                ...
                <?php if ($code != 'banner'): ?>
                    <div class="col-12">
                        ...
                        <textarea name="contents" id="contents_"
                            class="summernote"><?= esc($item['contents'] ?? '') ?></textarea>
                    </div>
                <?php endif; ?>
                            ...
                            <div class="col-md-6">
                                ...
                                <?php if (!empty($item['ufile6'])): ?>
                                    <div class="mt-2 p-2 border rounded bg-light">
                                        ...
                                        <a href="<?= base_url('data/bbs/' . $item['ufile6']) ?>" target="_blank"
                            ...
                            <div class="col-md-6">
                                ...
                                <?php if (!empty($item['ufile5'])): ?>
                                    <div class="mt-2 p-2 border rounded bg-light">
                                        ...
                                        <a href="<?= base_url('data/bbs/' . $item['ufile5']) ?>" target="_blank"
                                            class="small text-decoration-none">
                            ...
                            <div class="col-md-6">
                                ...
                                <?php if (!empty($item['ufile1'])): ?>
                                    <div class="mt-2 p-2 border rounded bg-light">
                                        ...
                                        <a href="<?= base_url('data/bbs/' . $item['ufile1']) ?>" target="_blank"
                                            class="small text-decoration-none">
                        ...
                        <?php if ($code == 'news' || $code == 'news_en'): ?>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="url" value="<?= esc($item['url'] ?? '') ?>"
